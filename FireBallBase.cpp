...
QTimer *FireBallBase::m_timer_static = nullptr;

FireBallBase::FireBallBase(QPointF pos_, qreal angle_, qreal speed_, qreal damage_, int penetration_, QGraphicsScene *scene, QObject *parent)
    : QObject{parent}, m_scene(scene), m_speed(speed_), m_damage(damage_), m_penetration(penetration_)
{
    ...
    // 计时器绑定
    connect(m_timer_static, &QTimer::timeout, this, &FireBallBase::enemyHitCheck);
    connect(m_timer_static, &QTimer::timeout, this, &FireBallBase::move);
...
void FireBallBase::enemyHitCheck(){
